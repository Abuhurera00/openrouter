...
import { useGetModels } from "@/utils/api/models/queries";
...
export function Landing() {
    const { data: modelsData, isLoading: _isModelsLoading } = useGetModels();

    const modelCount = modelsData?.data?.models.length ?? 0;
    ...
    return (
        <div className="min-h-screen bg-background text-foreground">
                ...
                <div className="relative max-w-6xl mx-auto px-6 text-center">
                    ...
                    <div className="flex items-center justify-center gap-4 mt-10">
                        ...
                        <Button variant="outline" size="lg" asChild className="h-12 px-8 text-base">
                            <Link to={PATH.DASHBOARD}>View dashboard</Link>
                        </Button>
                    </div>
            ...
            {/* Models preview */}
            {modelsData?.data?.models && modelsData.data.models.length > 0 && (
                <section className="py-24 border-t border-border/30">
                    <div className="max-w-6xl mx-auto px-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {modelsData.data.models.slice(0, 9).map((model: any) => (
                                <div
                                    key={model._id}
                                ...
                                >
                                    <div className="size-8 rounded-md bg-primary/5 border border-border/50 flex items-center justify-center text-xs font-bold text-muted-foreground">
                                        {model.companyId.name.charAt(0)}
                                    </div>
                                    <div className="min-w-0">
                                        <p className="text-sm font-medium truncate">{model.name}</p>
                                        <p className="text-xs text-muted-foreground">{model.companyId.name}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
