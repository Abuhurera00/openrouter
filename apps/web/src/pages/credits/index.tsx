...
import { useGetApiKeys } from "@/utils/api/apiKeys/queries";
import { useOnRampCreditsMutation } from "@/utils/api/payments/mutation";
import { API_KEYS_TAGS } from "@/utils/api/apiKeys/tags";
import { toast } from "sonner";
import type { ApiResponse } from "@/types/auth";
import { useGetUserProfile } from "@/utils/api/auth/queries";
import { AUTH_TAGS } from "@/utils/api/auth/tags";

export default function Credits() {
    const queryClient = useQueryClient();

    const { data: apiKeysData, isLoading: isApiKeysLoading } = useGetApiKeys();
    const { data: userProfileData, isLoading: isUserProfileLoading } = useGetUserProfile();

    const onrampMutation = useOnRampCreditsMutation({
        onSuccess: (data: ApiResponse) => {
            queryClient.invalidateQueries({ queryKey: [API_KEYS_TAGS.GET] });
            queryClient.invalidateQueries({ queryKey: [AUTH_TAGS.PROFILE] });
    ...
    const totalCreditsUsed = apiKeys.reduce(
        (sum: number, k: ApiKeyData) => sum + (k.credisConsumed ?? 0),
        0
    );
    const credits = userProfileData?.data?.credits;
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {onrampMutation.isSuccess && onrampMutation.data.data && (
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-3xl font-bold tracking-tight">
                                        {onrampMutation.data.data.totalCredits?.toLocaleString() ?? "—"} credits
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <CardContent>
                        {isApiKeysLoading ? (
                            ...
                            <div className="space-y-2">
                                {apiKeys.slice(0, 4).map((key: ApiKeyData) => (
                                    <div key={key._id} className="flex items-center justify-between text-sm">
                ...
                <CardContent>
                    ...
                    {onrampMutation.isSuccess && (
                        <div className="flex items-start gap-2.5 text-sm text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3.5 py-3 mt-4">
                            ...
                            <span>
                                1,000 credits added successfully! Your new balance: {onrampMutation?.data.data?.totalCredits?.toLocaleString() ?? "—"} credits.
                            </span>
                        </div>
                    )}
